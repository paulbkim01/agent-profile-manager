...
for config_file in "${SHELL_CONFIGS[@]}"; do
    if [ -f "$config_file" ] && grep -q 'apm init' "$config_file" 2>/dev/null; then
        # Remove lines containing 'apm init' and the '# Added by apm' comment
        sed -i.bak '/# Added by apm$/d' "$config_file"
        sed -i.bak '/apm init/d' "$config_file"
        rm -f "${config_file}.bak"
        echo "Removed shell integration from ${config_file}"
    fi
done
